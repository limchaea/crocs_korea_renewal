export const menuList = [
    {
        key: 'all',
        label: 'ALL',
        submenu_list: [
            { key: 'new', label: 'NEW', imgUrl: './images/item_01.png' },
            { key: 'women', label: 'WOMEN', imgUrl: './images/item_01.png' },
            { key: 'men', label: 'MEN', imgUrl: './images/item_01.png' },
            { key: 'kids', label: 'KIDS', imgUrl: './images/item_01.png' },
            { key: 'jibbitz', label: 'JIBBITZ', imgUrl: './images/item_01.png' },
            { key: 'collabs', label: 'COLLABS', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'new',
        label: 'NEW',
        submenu_list: [
            { key: 'fuzz', label: 'FUZZ', imgUrl: './images/item_01.png' },
            { key: 'jibbitz', label: 'JIBBITZ', imgUrl: './images/item_01.png' },
            { key: 'collabs', label: 'COLLABS', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'women',
        label: 'WOMEN',
        submenu_list: [
            { key: 'clog', label: '클로그', imgUrl: './images/item_01.png' },
            { key: 'platform', label: '플랫폼', imgUrl: './images/item_01.png' },
            { key: 'sneakers', label: '플랫 / 스니커즈 / 로퍼', imgUrl: './images/item_01.png' },
            { key: 'sandal', label: '플립 / 슬라이드 / 샌들', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'men',
        label: 'MEN',
        submenu_list: [
            { key: 'new', label: 'NEW', imgUrl: './images/item_01.png' },
            { key: 'clog', label: '클로그', imgUrl: './images/item_01.png' },
            { key: 'sandal', label: '샌들', imgUrl: './images/item_01.png' },
            { key: 'sneakers', label: '스니커즈', imgUrl: './images/item_01.png' },
            { key: 'echo', label: '에코', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'kids',
        label: 'KIDS',
        submenu_list: [
            { key: 'clog', label: '클로그', imgUrl: './images/item_01.png' },
            { key: 'boots', label: '부츠 / 레인부츠', imgUrl: './images/item_01.png' },
            { key: 'sandal', label: '샌들 / 플랫', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'jibbitz',
        label: 'JIBBITZ',
        submenu_list: [
            { key: 'all', label: 'ALL', imgUrl: './images/item_01.png' },
            { key: 'new', label: 'NEW', imgUrl: './images/item_01.png' },
            { key: 'pack', label: 'PACK', imgUrl: './images/item_01.png' },
            { key: 'single', label: 'SINGLE', imgUrl: './images/item_01.png' },
        ],
    },
    {
        key: 'collabs',
        label: 'COLLABS',
        submenu_list: [
            { key: 'all', label: '아티스트', imgUrl: './images/item_01.png' },
            { key: 'new', label: '애니메이션', imgUrl: './images/item_01.png' },
            { key: 'pack', label: '셀럽', imgUrl: './images/item_01.png' },
        ],
    },
];
